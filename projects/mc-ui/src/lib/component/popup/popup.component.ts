...
import { Component, Input, ElementRef, HostBinding, TemplateRef, EventEmitter, Output } from '@angular/core';
...
export class PopupComponent extends BaseComponent {
  ...
  show(resizeOnly = false) {
    ...
    setTimeout(() => {
      ...
      // indicator location
      const isLeft =
        targetSize.left + (this.startFrom === 'center' ? targetSize.width / 2 : 0) + popupSize.width + this.offsetX <=
        windowSize.width;
      const isTop =
        targetSize.top + targetSize.height + popupSize.height + this.offsetY + indicatorHeight <= windowSize.height;
      const left = isLeft
        ? targetSize.left + (this.startFrom === 'center' ? targetSize.width / 2 : 0) + this.offsetX
        : targetSize.left -
        popupSize.width +
        (this.startFrom === 'center' ? targetSize.width / 2 : targetSize.width) -
        this.offsetX;
      const top = isTop
        ? targetSize.top + (this.startFrom === 'overlap' ? 0 : targetSize.height) + this.offsetY + indicatorHeight
        : targetSize.top -
        popupSize.height -
        this.offsetY -
        indicatorHeight +
        (this.startFrom === 'overlap' ? targetSize.height : 0);
